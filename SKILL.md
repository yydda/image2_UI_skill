...
On Windows, never launch a `.ps1` path directly, never use `Start-Process <script>.ps1`, and never ask the user to open a `.ps1` file through Explorer or the Open With dialog. Use the `.cmd` wrappers first:
...
If a `.ps1` script must be called directly, invoke it only with:
