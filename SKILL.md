...
4. 高保真确定性检查：使用本仓库脚本做 preflight、CSS box-model planning、contact sheet、theme calibration、crop、repair、score、diff diagnosis、DOM audit、repair queue 和 fidelity loop。
5. 可复用工程基座：优先拉取 `moni-ui-foundation` 的最新模板、tokens、primitives 和 fidelity helpers；本 skill 只负责编排、校验和沉淀建议。
6. 不接 Figma 路径；本 skill 只服务“截图/图片输入 -> 前端代码”工作流。
...
## Foundation Memory Loop

`moni-ui-foundation` 是长期复用的前端工程基座；`moni-ui-skill` 是编排层。不要把每次任务的一次性页面都塞回 skill，也不要无审查地污染 foundation。

任务开始：

- 运行 `node scripts/sync-foundation.mjs` 拉取或更新 foundation；如果失败，使用内置模板 fallback，并在最终报告说明。
- 新 demo 使用 `node scripts/scaffold-react-project.mjs --target <target>`，不要手搓工程结构。
- 优先复用 foundation 中的 tokens、theme presets、primitives、layout、fidelity helpers 和验证脚本。

任务结束：

- 对新 demo 或较大的页面实现运行 `node scripts/generate-reuse-review.mjs --project <project>`。
- 阅读 `tmp/fidelity/reuse-review.md`，把可沉淀的通用组件、tokens、脚本和一次性页面代码分开。
- 只有用户明确同意沉淀时，才运行 `node scripts/promote-to-foundation.mjs --project <project> --paths <relative-paths> --apply`，然后在 `moni-ui-foundation` 中单独提交。
- 禁止自动沉淀客户 logo、截图裁切、生成图片、项目文案、业务数据、本机绝对路径或只服务单一页面的像素补丁。

治理规则见 `references/foundation-governance.md`。
...
React/shadcn 新建、初始化或补齐细节见 `references/react-shadcn-workflow.md`。统一架构检查见 `references/frontend-architecture-contract.md`。foundation 拉取、复盘和沉淀治理见 `references/foundation-governance.md`。

## Windows Script Execution Guard

On Windows, expose `.cmd` wrappers as the default user-facing entrypoints. Keep `.ps1` files as internal implementations only. Never launch a `.ps1` path directly, never use `Start-Process <script>.ps1`, and never ask the user to open a `.ps1` file through Explorer or the Open With dialog. Use the `.cmd` wrappers first:

```bat
.\validate.cmd
.\scripts\install-local.cmd
.\scripts\sync-local.cmd -Clean
.\scripts\setup-fidelity-tools.cmd -CheckOnly
```

If a `.ps1` script must be called directly as an implementation fallback, invoke it only with:
...
- 高保真素材必须先定义 CSS slot，再接入图片；slot 属于组件盒模型，图片文件属于资产流水线。不要用图片自然尺寸反推布局。
- 精确插图、线稿、logo、支付图标和半透明装饰默认导出为高清透明 PNG/WebP 或 SVG；要求透明时必须保留 alpha，不要使用白底或米色底冒充透明。
- 半透明线稿或浅色装饰如果透明化会破坏线条，必须标记 `backgroundMatched` 和采样背景色；否则输出 alpha PNG，并在深浅背景上检查脏边、白边和硬边。
...
当用户要求 1:1、高保真、像素级复刻，或明确指出 image_gen 生成了不相干图片时，默认进入高保真素材修复流程：**CSS 盒模型规划 + 截图裁切 + 本地修复 + 元素清单 + 图标清单 + 字体包接入 + 评分验收 + image_gen 兜底**。
...
- 先运行 `node scripts/validate-fidelity-plan.mjs --blueprint tmp/fidelity/page-blueprint.json --layout tmp/fidelity/layout-manifest.json --assets assets.manifest.json --elements tmp/fidelity/element-manifest.json --icons tmp/fidelity/icon-inventory.json --interactions tmp/fidelity/interaction-map.json --mode strict --fail-on-error`；失败时先修计划，不进入实现。
- 页面蓝图必须描述 canvas、major regions、text layers、asset slots、interaction targets 和 known risks；布局 manifest 必须给每个主要区域绑定组件、CSS box model、padding/margin/gap/border/radius/shadow 和实现责任；element manifest 必须记录关键按钮、文本、图标、分割线、价格、状态项的 x/y/width/height/font/selector；icon inventory 必须覆盖所有自定义图标。
- React 实现前必须运行 `node scripts/build-asset-contact-sheet.mjs --manifest assets.manifest.json --source tmp/fidelity/clean-reference.png --fail-on-review`，查看 `tmp/fidelity/asset-contact-sheet.html`，确认所有 exact 资产都有正确 crop/repair/vector 策略、2x 目标、alpha 策略和预览；如果仍有 `needs-repair`、`needs-review`、缺 alpha、低于 2x 或 backgroundMatched 未记录背景色，不得进入页面实现。
- 新主题或颜色/字体明显不准时，先运行 `node scripts/calibrate-theme.mjs --reference tmp/fidelity/clean-reference.png --elements tmp/fidelity/element-manifest.json --assets assets.manifest.json`，把输出作为 `src/theme/themes/*.css` 的起点，再以设计判断微调。必须传 `--assets` 排除 logo、支付图标、插画和线稿 crop 区域，避免支付宝蓝、微信绿、插画色污染品牌色和边框色。
...
- 素材输出默认按槽位至少 2x，复杂半透明素材优先 PNG-32；需要更小体积时可额外导出 WebP alpha，但保留 PNG 源。严禁把半透明素材导出为带底色 JPEG。
...
- 如果把资产从 2x/`upscale` 降为 1x/`none`，必须写 `densityPolicy: "source-1x-accepted"`、`downgradeReason` 和证据；不得无说明降低质量门槛。
- 批量裁切后必须先运行 `node scripts/repair-asset.mjs --manifest assets.manifest.json` 和 `node scripts/score-asset.mjs --manifest assets.manifest.json --fail-on-reject`。通过评分后，更新 manifest 中对应资产 `status: "accepted"`；然后再次运行 `validate-fidelity-plan.mjs --enforce-asset-acceptance --fail-on-error`。未 accepted 的 exact 资产不得接入 React。
...
- diff、DOM audit 或 asset score 失败后，必须先运行 `diagnose-fidelity-diff.mjs`，再运行 `build-repair-queue.mjs --diagnosis-report tmp/fidelity/diff-diagnosis.json` 和 `run-fidelity-loop.mjs --diagnosis tmp/fidelity/diff-diagnosis.json --max-iterations 6`，每轮只修 worst/focus 区域，最多 6 轮后仍失败则降级交付状态。
- 字体校准必须进入截图循环：先按 element manifest 记录标题、正文、数字、按钮、协议文案等字体预期；如果系统字体不接近，就联网查找官方/开源字体包，下载或安装后放入 `src/assets/fonts/`，通过 `src/theme/font-faces.css` 的 `@font-face` 引入；再运行 DOM 审计，调整 `font-family`、`font-size`、`font-weight`、`line-height`，直到关键文本层级接近参考图或明确记录不可达原因。
...
高保真强制执行顺序和四件套 schema 见 `references/high-fidelity-execution-contract.md`。CSS 盒模型规划、组件化与像素眼精调见 `references/box-model-fidelity-workflow.md`。高保真资产 manifest 字段、修复策略、评分规则和生图兜底 prompt 见 `references/fidelity-asset-repair.md`。asset contact sheet、diff diagnosis、theme calibration、repair loop、组件库和速度规则见 `references/high-fidelity-iteration-tools.md`。真实测试中发现的工作流问题、外部方案调研和下一步优化清单见 `references/high-fidelity-workflow-observations.md`。
...
5. 高保真模式下先输出并校验 `page-blueprint.json`、`layout-manifest.json`、`assets.manifest.json`、`element-manifest.json`、`icon-inventory.json`、`interaction-map.json`；layout manifest 必须用 CSS 盒模型拆页面，明确 grid/flex/block、padding、margin、gap、border、radius、shadow、slot 和组件 owner；普通模式下至少输出资产清单。资产清单要包含资产 id、UI 位置、目标槽位尺寸、导出尺寸、宽高比、`sourceStrategy`、`repairStrategy`、`qualityGate`、后处理需求、集成目标，以及是否允许 `image_gen-fallback`。
6. 只生成真正需要位图生成的资产。结构性 UI、可读文字和普通控件继续用代码实现；1:1 精确资产先裁切/修复/矢量重建。
7. 对需要统一风格的一组资产，优先考虑“一次生成统一资产板，再切片导出”的方案，减少风格漂移。
8. 按需做后处理：裁剪、缩放、去背景、添加 alpha、压缩和尺寸验证；高保真资产必须先运行 contact sheet、本地修复与评分脚本，且使用 `--fail-on-review`、`--fail-on-reject`、`--enforce-asset-acceptance` 等硬闸门，不合格则打回。
...
13. 将最终页面截图与原始 UI 参考图做差距核对，列出差异，修正后再次截图对比；失败时用 `diagnose-fidelity-diff.mjs`、`build-repair-queue.mjs --diagnosis-report tmp/fidelity/diff-diagnosis.json`、`run-fidelity-loop.mjs --diagnosis tmp/fidelity/diff-diagnosis.json --max-iterations 6` 进入下一轮。
14. 新 demo 或较大页面实现结束时运行 `node scripts/generate-reuse-review.mjs --project <project>`，列出可沉淀到 `moni-ui-foundation` 的候选项；未经用户确认不要自动 promote。
15. 如果目标是前端应用，最后用渲染截图和点击路径验证效果，并确认截图里真实出现了 image_gen 资产。
...
5. 如果用户允许联网查找字体，优先寻找官方字体源、开源字体仓库或字体厂商页面，并检查授权范围。中文常见候选包括 Noto Sans/Serif SC、Source Han Sans/Serif SC、思源黑体/宋体等开源字体；商业或品牌字体必须由用户提供授权文件。
6. 下载或安装字体后，优先自托管到 `src/assets/fonts/`，在 `src/theme/font-faces.css` 用 `@font-face` 引入，再通过 `src/theme/tokens.css` 的 `--font-sans`、`--font-serif`、`--font-mono` 接入。
7. 加载字体时优先使用 `woff2`，设置合理的 `font-display`，并提供 fallback 字体栈，避免字体加载失败后页面崩坏。

集成字体时：

- 使用真实文本和 CSS 字体，不把可编辑 UI 文案烘焙进图片。
- 字体 CSS 必须被 `src/index.css` 或项目主样式入口 import；不要只把字体文件放进目录但没有 CSS 引用。
...
- 新 demo 或较大页面实现的 `reuse-review.md` 路径，以及哪些候选项建议沉淀到 `moni-ui-foundation`、哪些必须保留为一次性项目代码。
