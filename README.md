...
- `scripts/scaffold-react-project.mjs`：从统一模板创建新 React 项目，避免临时手搓工程结构。
...
不要重建工程架构；新 demo 使用统一模板、tokens、theme presets、src/components/ui/primitives/layout/fidelity 分层，并运行 architecture:check。
...
新建项目必须使用统一模板或 scripts/scaffold-react-project.mjs，不要重建工程架构；风格差异只通过 src/theme/tokens.css、src/theme/themes/*.css 和页面实现变化。
请判断哪些区域用代码实现，哪些区域需要调用内置 image_gen 生成图片资产。
生成资产放到 src/assets/generated，并通过 React import 接回组件。
优先调用 Codex Browser 做本地预览、截图、点击和 DOM 检查；最后运行 npm run architecture:check、typecheck/build，并提供截图验真。
