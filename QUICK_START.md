...
Windows 下不要双击 `.ps1` 文件；所有日常安装、同步和验证都优先运行 `.cmd`。也不要裸跑 `npm`、`npx`、`vite`、`tsc` 或 `playwright`；请使用 `cmd /c npm.cmd ...`、`cmd /c npx.cmd ...`，新建 React demo 优先用模板里的 `validate.cmd` 和 `dev.cmd`。
...
实现后必须运行：
- Windows：cmd /c validate.cmd
- 或依次运行：cmd /c npm.cmd run architecture:check
- cmd /c npm.cmd run typecheck
- cmd /c npm.cmd run build
- 启动预览：node scripts\start-dev-server.mjs --port 5173 或 cmd /c dev.cmd --port 5173；如果首次启动 stdout 为空，读取 tmp/dev-server.json 里的 url
...
完成后在 Windows 下运行 cmd /c validate.cmd，或依次运行 cmd /c npm.cmd run architecture:check、cmd /c npm.cmd run typecheck、cmd /c npm.cmd run build；再用 node scripts\start-dev-server.mjs --port 5173 或 cmd /c dev.cmd --port 5173 启动本地预览；如果 stdout 为空，读取 tmp/dev-server.json 里的 url。检查主要点击路径，并提供截图验真。
...
```text
Use $moni-ui-skill，参考截图做成可点击 React demo，默认 Vite + React + TypeScript + shadcn；先同步 moni-ui-foundation，按 CSS 盒模型拆页面，精确素材走裁切/修复/矢量化，非精确资产才用 image_gen 兜底，字体不准就自托管官方/开源字体包。Windows 下验证用 cmd /c validate.cmd，启动预览用 node scripts\start-dev-server.mjs --port 5173 或 cmd /c dev.cmd --port 5173，stdout 为空就读 tmp/dev-server.json，不要裸跑 npm/vite/tsc/playwright；最后截图验真和 reuse review。
```
