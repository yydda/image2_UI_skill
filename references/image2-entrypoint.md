...
这份文档用于回答一个关键问题：当前项目里“image2”到底怎么调用。只有确认了项目指定入口，才可以进入真实生图闭环。
...
以下都不能当作 image2，除非用户明确允许作为替代方案：

- imagegen skill
- gpt-image 系列 CLI/API
...
如果使用了这些替代方案，最终必须明确写成“替代方案”，不能写成“已调用 image2”。

## 4. 找不到入口时怎么办

如果无法确认 image2 入口：

1. 继续完成 UI 拆解、资产清单、提示词和代码骨架。
2. 明确标注“尚未完成真实 image2 生图”。
3. 不要生成或接入其他模型图片来冒充 image2。
4. 最终列出待执行的 image2 资产和提示词，方便用户补充入口后继续。

可使用这段说明：

```text
当前环境无法确认项目指定 image2 调用入口。我已经完成 UI 拆解、资产清单和提示词准备，但尚未完成真实 image2 生图。为了避免误报，我没有使用 imagegen、OpenRouter、gpt-image 或自写 OpenAI SDK 脚本替代 image2。
```
